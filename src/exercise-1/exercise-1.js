export default function app(invoice, vehicles) {
  ...
  const format = new Intl.NumberFormat('pl-PL', { style: 'currency', currency: 'PLN', minimumFractionDigits: 2, }).format;
  for (let ride of invoice.rides) { const vehicle = vehicles[ride.vehicleId];
    let thisAmount = 0;
    switch (vehicle.type) {
      case 'eco':        thisAmount = 500;
        if (ride.minutes > 30) {
          thisAmount += 20 * (ride.minutes - 30);
        }
        break;
      case 'electric':        thisAmount = 1000;
        if (ride.minutes > 10) {
